...
const MOCK_MODULE_DEFINITIONS: Array<Omit<ModuleStatus, 'active' | 'health'> & { health?: ModuleStatus['health'] }> = [
  ...
  },
  {
    ...
    display_name: 'Codex 状态复用',
    description: '在 (账号, 模型) 桶内复用官方 Codex Turn-State，绕开降级态（降智），支持代理池探测续期。',
    category: 'integration',
    available: true,
    enabled: true,
    config_validated: true,
    config_error: null,
    admin_route: '/admin/modules/codex-turn-state',
    admin_menu_icon: 'Recycle',
    admin_menu_group: null,
    admin_menu_order: 59,
  },
  {
